...
class ClienteControllerSwagger extends Controller
{
        ...
        requestBody: new OA\RequestBody(
            ...
            content: new OA\JsonContent(
                ...
                properties: [
                    ...
                    new OA\Property(property: 'email_contacto', type: 'string', example: 'maria@comercialejemplo.cl'),
                ]
            )
        ),
        responses: [
    ...
    public function store(Request $request)
    {
        $datos = $request->validate([
            'rut_empresa' => ['required', 'string', 'regex:/^\d{7,8}-[0-9kK]$/', 'unique:clientes,rut_empresa'],
            ...
            'rut_empresa.regex' => 'El rut debe tener el formato 12345678-9 (sin puntos).',
            'email_contacto.regex' => 'El correo de contacto debe tener un formato válido.',
        ]);
    ...
    {
        $datos = $request->validate([
            'rut_empresa' => ['required', 'string', 'regex:/^\d{7,8}-[0-9kK]$/', Rule::unique('clientes', 'rut_empresa')->ignore($cliente->id)],
            'rubro' => ['required', Rule::in(Cliente::RUBROS)],
            'razon_social' => ['required', 'string', 'max:150'],
            'telefono' => ['required', 'string', 'max:20'],
            'direccion' => ['required', 'string', 'max:255'],
            ...
            'rut_empresa.regex' => 'El rut debe tener el formato 12345678-9 (es solo que queria colocarlo asi)',
            'email_contacto.regex' => 'El correo de contacto debe tener un formato válido.(y bueno me estoy quedando sin cosas para escribir)',
        ]);
